...
class BookInStock
  ...
  def initialize(isbn, price)
    raise ArgumentError.new("Invalid isbn provided, got #{isbn}") if isbn.nil? || isbn.empty?
    raise ArgumentError.new("Price must be a positive number, got #{price}") if price.nil? || price <= 0
